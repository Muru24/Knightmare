...
public class DungeonCrawlerController : Singleton<DungeonCrawlerController>
{
    ...
    public void AddShopRoom()
    {
        ...
        bool selectShopRoomStatus = false;

        for (int idx = validRoomList.Count - 1; 0 < idx; idx--)
        {
            if (!selectShopRoomStatus)
            {
                int setLIstCnt = idx;
                Vector3Int pos = validRoomList[setLIstCnt].center_Position;
                ...
                for (int i = 0; i < direction4.Count; i++)
                {
                    selectShopRoomStatus = false;
                    Vector3Int ShopRoomPos = posArr[pos.z, pos.x].center_Position + direction4[i];

                    if (PossibleArr(ShopRoomPos))
                    {
                        if ((AroundRoomCount(ShopRoomPos) < 2)
                            && !posArr[ShopRoomPos.z, ShopRoomPos.x].isValidRoom)
                        {
                            posArr[ShopRoomPos.z, ShopRoomPos.x].roomName = "Shop";
                            posArr[ShopRoomPos.z, ShopRoomPos.x].isValidRoom = true;
                            posArr[ShopRoomPos.z, ShopRoomPos.x].center_Position = ShopRoomPos;
                            posArr[ShopRoomPos.z, ShopRoomPos.x].parent_Position = ShopRoomPos;
                            posArr[ShopRoomPos.z, ShopRoomPos.x].mergeCenter_Position = ShopRoomPos;
                            posArr[ShopRoomPos.z, ShopRoomPos.x].distance = posArr[pos.z, pos.x].distance + 1;
                            posArr[ShopRoomPos.z, ShopRoomPos.x].roomType = "Single";

                            selectShopRoomStatus = true;
                            break;
                        }
                    }
                }
    ...
    public void AddMiddleBossRoom()
    {
        ...
        bool selectMiddleBossRoomStutus = false;

        for (int idx = validRoomList.Count - 1; 0 < idx; idx--)
        {
            if (!selectMiddleBossRoomStutus)
            {
                int setLIstCnt = idx;
                Vector3Int pos = validRoomList[setLIstCnt].center_Position;
                ...
                for (int i = 0; i < direction4.Count; i++)
                {
                    selectMiddleBossRoomStutus = false;
                    Vector3Int MiddleBossRoom = posArr[pos.z, pos.x].center_Position + direction4[i];

                    if (PossibleArr(MiddleBossRoom))
                    {
                        if ((AroundRoomCount(MiddleBossRoom) < 2)
                            && !posArr[MiddleBossRoom.z, MiddleBossRoom.x].isValidRoom)
                        {
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].roomName = "MiddleBossRoom";
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].isValidRoom = true;
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].center_Position = MiddleBossRoom;
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].parent_Position = MiddleBossRoom;
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].mergeCenter_Position = MiddleBossRoom;
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].distance = posArr[pos.z, pos.x].distance + 1;
                            posArr[MiddleBossRoom.z, MiddleBossRoom.x].roomType = "Single";

                            selectMiddleBossRoomStutus = true;
                            break;
                        }
                    }
                }
